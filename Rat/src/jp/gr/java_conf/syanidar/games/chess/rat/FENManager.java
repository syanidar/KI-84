...
public class FENManager {
	...
	public FENManager(String arg){
		...
		if(st.countTokens() != 6)
			throw new IllegalArgumentException("For input string = " + arg);
		placement = st.nextToken();
		color = st.nextToken();
		if(color.length() != 1 || !(color.equals("w") || color.equals("b")))
			throw new IllegalArgumentException("color = " + color);
		castlingAvailability = st.nextToken();
		if(castlingAvailability.length() == 0)
			throw new IllegalArgumentException("castling availability = " + "\"\"");
		enPassantSquare = st.nextToken();
		if(!enPassantSquare.equals("-") && enPassantSquare.length() != 2)
			throw new IllegalArgumentException("en passant target square = " + "\"\"");
		halfMoveClock = st.nextToken();
		if(!isNaturalNumber(halfMoveClock))
			throw new IllegalArgumentException("half move clock = " + halfMoveClock);
		fullMoveNumber = st.nextToken();
		if(!isNaturalNumber(fullMoveNumber))
			throw new IllegalArgumentException("full move number = " + fullMoveNumber);
		fen = arg;
	}
	...
	public final Position newPosition(){
		...
		if(!castlingAvailability[K]){
			pb = pb.disableWhiteKingsideCastling();
		}
		if(!castlingAvailability[Q]){
			pb = pb.disableWhiteQueensideCastling();
		}
		if(!castlingAvailability[k]){
			pb = pb.disableBlackKingsideCastling();
		}
		if(!castlingAvailability[q]){
			pb = pb.disableBlackQueensideCastling();
		}
	...
	private final long[][] army(){
		...
		for(int i : Range.of(chars.length)){
			...
			switch(chars[i]){
			case 'P':
				result[WHITE][PAWN] |= square;
				index--;
				break;
			case 'N':
				result[WHITE][KNIGHT] |= square;
				index--;
				break;
			case 'B':
				result[WHITE][BISHOP] |= square;
				index--;
				break;
			case 'R':
				result[WHITE][ROOK] |= square;
				index--;
				break;
			case 'Q':
				result[WHITE][QUEEN] |= square;
				index--;
				break;
			case 'K':
				result[WHITE][KING] |= square;
				index--;
				break;
			case 'p':
				result[BLACK][PAWN] |= square;
				index--;
				break;
			case 'n':
				result[BLACK][KNIGHT] |= square;
				index--;
				break;
			case 'b':
				result[BLACK][BISHOP] |= square;
				index--;
				break;
			case 'r':
				result[BLACK][ROOK] |= square;
				index--;
				break;
			case 'q':
				result[BLACK][QUEEN] |= square;
				index--;
				break;
			case 'k':
				result[BLACK][KING] |= square;
				index--;
				break;
			default:
				if(!Character.isDigit(chars[i]) && chars[i] != '/')throw new IllegalArgumentException("piece placement = " + placement);
				if(chars[i] != '/'){
					index -= (int)(chars[i] - '0');
				}
				break;
	...
	private final boolean[] castlingAvailability(){
		...
		for(char c : castlingAvailability.toCharArray()){
			...
			case '-':break;
			default:
				throw new IllegalArgumentException("castling availability = " + castlingAvailability);
			}
		}
		return result;
	}
