...
public class ChessUtility {
	...
	public static final Player<Position, MinimaxResult<CentiPawn>> ai(int depth){
	...
	public static final Player<Position, MinimaxResult<CentiPawn>> ai(int minDepth, int maxDepth, Predicate<Position> predicate){
		MaterialEvaluator he = MaterialEvaluator.getInstance();
		AlphaBetaAnalyzer<Position, CentiPawn> aba = new AlphaBetaAnalyzer<>(he);
		AlphaBetaSetting<Position, CentiPawn> abs = new AlphaBetaSetting<>(minDepth - 1, maxDepth, he, predicate);
		return new AIPlayer<>(aba, abs, he);
	}
	public static final Player<Position, MinimaxResult<CentiPawn>> human(MoveSelector selector){
